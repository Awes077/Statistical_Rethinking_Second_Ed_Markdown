...
#section 4.1.1 - Normal by addition
...
#Gonna take another swing at this before I actually do the spline model. So each column in B is a basis function. Each row is a year. Each ROW sums to 1, not column. Columns sum to a different value. Is it always the same? Nope. So each basis function is some sum of years? The overall impact of EVERY year across basis functions is 1. So we are taking
#our large number of years and creating 17 new variables. We then sum these variables to get a new predictor. Each basis should be the same structurally but seems to represent
#a different relationship among specific years. We then weight each of these composite sums using the beta coefficients in a model. So like...if I start with a data point that
#has a year of 1200, what does its spline predictor become? I want to predict number of days into the year that we blossom I think. Might have had too much coffee to unravel this today. Just gonna jump to the model now.
d2$year
sum(B[1,])
B[1,]
B[,2]
...
length(na.omit(cherry_blossoms$doy))
```
...
#alright this plot gives us the basis times the weights, so our predicted value minus the intercept. Right? Yeah. Sensible.
...
#now we plot the actual predictions against obseved data I believe.
...
#once again, understanding this is gonna require some actual heavy thinking on my part but I'll get around to it eventually.
...
4E1) Likelihood is $y_{i} \sim Normal(\mu, \sigma)$
4E2) 2 parameters, $\mu$ and $\sigma$
4E3) $P(\mu,\sigma | y) = \frac{P(y_{i} | \mu, \sigma) P(\mu)P(\sigma)}{P(y)}$ Dunno if that's 100% correct but it's what I'm runnin with
4E4) line 2, $\mu_{i} = \alpha +\beta x_{i}$
4E5) 3, $\alpha$, $\beta$, and $\sigma$

4M1) Gonna sim from our priors ugh

4M2) now convert to a quap

```{r}

#4M1
mu_samp <- rnorm(1e4, 0, 10)
sig_samp <- rexp(1e4, 1)

y_samp <- rnorm(mu_samp, sig_samp)
